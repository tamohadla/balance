...
const source=(await readFile(new URL('../js/item-image-store.js',import.meta.url),'utf8'))
 ...
 .replace(/^import .*image-variants.*$/m,'const resizeImageBlob=async(b,n)=>({size:n}), thumbnailPath=p=>p+".thumb-224.jpg";');
const {saveImagePair,removeImagePair}=await import('data:text/javascript;base64,'+Buffer.from(source).toString('base64'));
function fixture(mode=''){
 const state={path:'items/old.jpg',files:new Set(['items/old.jpg','items/old.jpg.thumb-224.jpg','items/old.jpg.thumb-200.jpg']),uploads:0};
...
test('replacement publishes both files, removes old pair, and deletion removes both versions',async()=>{
 ...
 assert.equal(state.path,path);assert.deepEqual([...state.files].sort(),[path,path+'.thumb-224.jpg'].sort());
...
for(const mode of ['upload','db','conflict'])test(`${mode} failure retains previous full image and thumbnail`,async()=>{
 ...
 assert.equal(state.path,'items/old.jpg');assert.deepEqual([...state.files].sort(),['items/old.jpg','items/old.jpg.thumb-200.jpg','items/old.jpg.thumb-224.jpg']);
});
test('lost response after commit never deletes the newly referenced pair',async()=>{
 ...
 assert.equal(state.path,path);assert.ok(state.files.has(path));assert.ok(state.files.has(path+'.thumb-224.jpg'));
});
test('unknown commit outcome retains files until status can be determined',async()=>{
 const {client,state}=fixture('unknown');await assert.rejects(saveImagePair(client,'id',state.path,{}));assert.equal(state.files.size,5);
});
