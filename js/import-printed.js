...
/**
 * 1. ميزة الإكمال التلقائي (Autocomplete)
 * تجلب المجموعات من جدول المواد الحالي لتعرضها كاقتراحات
 */
...
function fillDatalist(id, set, input) {
    ...
    // تحويل الـ Set لخيارات واضحة للمتصفح
...
// مفتاح المطابقة: اسم المادة + الكود فقط (كما طلبت)
...
btnFetch.onclick = async () => {
    ...
    try {
        // أ. جلب المخزون الحالي (الاسم واللون فقط للمطابقة)
        ...
        const { data: sourceData, error } = await q.limit(2000);
        if (error) throw error;

        // ج. معالجة البيانات وإزالة التكرار
        const map = new Map();
...
// تشغيل Autocomplete فور تحميل الملف
setupAutocomplete();
