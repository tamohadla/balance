...
const keyOf = (d) => `${d.main_category}||${d.sub_category}||${d.item_name}||${d.color_code}`.toLowerCase();

async function fetchExistingKeys(candidates) {
    const mains = [...new Set(candidates.map(x => x.main_category))];
    const { data } = await supabase.from("items").select("main_category, sub_category, item_name, color_code").in("main_category", mains);
    return new Set((data || []).map(r => keyOf(r)));
}
