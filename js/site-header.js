import {canAccessPage,homePage,ROLE_LABELS} from './permissions.js?v=1';
const mainLinks = [
...
export function buildHeader(access, logout) {
  ...
  const brand=link(homePage(access.member),'','site-brand');brand.setAttribute('aria-label','Balance — الرئيسية');
  ...
  user.append(el('small','',ROLE_LABELS[access.member.role]));
  ...
  for(const [href,text,icon] of mainLinks){if(!canAccessPage(access.member,href))continue;const a=link(href,'','site-nav__link');const i=el('span','site-nav__icon',icon);i.setAttribute('aria-hidden','true');a.append(i,el('span','',text));nav.append(a);}
  for(const [title,allLinks] of groups){const links=allLinks.filter(([href])=>canAccessPage(access.member,href));if(!links.length)continue;const details=el('details');const summary=el('summary','',title),menu=el('div','site-nav__menu');
