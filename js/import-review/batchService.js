...
export async function loadActiveItems() {
  const { data, error } = await supabase
    .from("items")
    .select(
      "id, main_category, sub_category, item_name, color_code, color_name, unit_type, image_path",
    )
    .eq("is_active", true)
...
export async function createBatchWithLines({
  batchType,
  fileName,
  lines,
  notes,
...
export async function recomputeBatchStats(batchId) {
  const lines = await allRows(() =>
    supabase
      .from("import_batch_lines")
      .select("id, match_status, is_included, is_posted")
      .eq("batch_id", batchId)
      .order("id"),
  );
  ...
  const stats = {
    ...
    exact_matched_lines: lines.filter(
      (x) => x.match_status === MATCH_STATUS.EXACT,
    ).length,
    suggested_lines: lines.filter(
      (x) => x.match_status === MATCH_STATUS.SUGGESTED,
    ).length,
    manual_matched_lines: lines.filter(
      (x) => x.match_status === MATCH_STATUS.MANUAL,
    ).length,
    unknown_lines: lines.filter((x) => x.match_status === MATCH_STATUS.UNKNOWN)
      .length,
    duplicate_warning_lines: lines.filter(
      (x) => x.match_status === MATCH_STATUS.DUPLICATE,
    ).length,
    excluded_lines: lines.filter((x) => !x.is_included).length,
    approved_lines_count: lines.filter((x) => x.is_posted).length,
    pending_lines_count: lines.filter((x) => x.is_included && !x.is_posted)
      .length,
  };

  let status = BATCH_STATUS.DRAFT;
  if (stats.approved_lines_count > 0 && stats.pending_lines_count > 0)
    status = BATCH_STATUS.PARTIAL;
  if (stats.approved_lines_count > 0 && stats.pending_lines_count === 0)
    status = BATCH_STATUS.APPROVED;

  const patch = { ...stats, status };
  if (status === BATCH_STATUS.APPROVED)
    patch.approved_at = new Date().toISOString();

  const { data: batch, error: uErr } = await supabase
    .from("import_batches")
    .update(patch)
    .eq("id", batchId)
    .select("*")
...
export async function loadBatch(batchId) {
  ...
  const lines = await allRows(() =>
    supabase
      .from("import_batch_lines")
      .select(
        "*, matched_item:items!import_batch_lines_matched_item_id_fkey(id, main_category, sub_category, item_name, color_code, color_name), suggested_item:items!import_batch_lines_suggested_item_id_fkey(id, main_category, sub_category, item_name, color_code, color_name)",
      )
      .eq("batch_id", batchId)
      .order("row_index", { ascending: true }),
  );
...
export async function detectDuplicates(batch) {
  ...
  for (const line of lines || []) {
    const itemId =
      ...
      (line.suggested_approved ? line.suggested_item_id : null);
    if (!itemId || !line.raw_date || !line.raw_qty_primary) continue;

    let q = supabase
      .from("stock_moves")
      .select(
        "id, move_date, qty_main_in, qty_main_out, qty_rolls_in, qty_rolls_out, note",
      )
    ...
    if (batch.batch_type === "purchase")
      q = q.eq("qty_main_in", line.raw_qty_primary);
    else q = q.eq("qty_main_out", line.raw_qty_primary);

    if (line.raw_rolls) {
      if (batch.batch_type === "purchase")
        q = q.eq("qty_rolls_in", line.raw_rolls);
...
export async function postReadyLines(batchId) {
  const { error } = await supabase.rpc("post_stock_import", {
    p_batch: batchId,
  });
  if (error) throw error;
